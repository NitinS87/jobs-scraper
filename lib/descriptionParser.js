...
function parseDescription(html) {
  ...
  const experience_level = detectExperienceLevel(plainText);

  const visa_sponsorship = detectVisaSponsorship(plainText);
...
// Phrases that REFUSE sponsorship. Checked before the offer patterns, because a
// refusal almost always contains the word "sponsorship" too.
//
// The previous implementation was a bare /visa\s*(?:sponsor|support)/ test, which
// matched the phrase wherever it appeared — including in a refusal. Measured
// 2026-09-24: of 400 jobs flagged true, 299 had a negation within 40 characters.
// Real rows marked as OFFERING sponsorship included "Visa Sponsorship — Employer
// will not sponsor applicants for employment" and "This position is not open for
// Visa sponsorship".
const VISA_REFUSED = [
  /\b(?:will|can|do|does|are)\s*n(?:o|')t\s+(?:be\s+)?(?:able\s+to\s+)?(?:offer|provide|consider|support|sponsor)\b[^.]{0,60}\b(?:visa|sponsor)/i,
  /\bno\b[^.]{0,30}\bvisa\s*(?:sponsorship|support)/i,
  /\bvisa\s*sponsorship\b[^.]{0,40}\b(?:is\s+)?not\s+(?:available|offered|provided|possible)/i,
  /\bnot\s+open\b[^.]{0,30}\bvisa/i,
  /\bunable\s+to\s+sponsor\b/i,
  /\bwithout\s+(?:the\s+need\s+for\s+)?(?:visa\s+)?sponsorship/i,
  /\bmust\s+(?:be\s+)?(?:legally\s+)?(?:authori[sz]ed|eligible)\s+to\s+work\b[^.]{0,60}\bwithout\b/i,
  /\bdoes\s+not\s+sponsor\b/i,
  // "Employer will not sponsor applicants for employment" — the refusal verb
  // stands alone, with no second mention of "visa" after it.
  /\b(?:will|would|can|do|does|are)\s*n(?:o|')?t\s+(?:be\s+able\s+to\s+)?sponsor\b/i,
  /\bsponsorship\s+(?:is\s+)?not\s+(?:available|offered|provided)/i,
  /\bno\s+sponsorship\b/i,
];

// Phrases that OFFER it.
const VISA_OFFERED = [
  /\bvisa\s*(?:sponsorship|support)\s*(?:is\s+)?(?:available|offered|provided|possible)/i,
  /\b(?:we|company)\s+(?:can\s+|will\s+|do\s+)?(?:offer|provide|sponsor)\b[^.]{0,40}\bvisa/i,
  /\bsponsorship\s+(?:is\s+)?available/i,
  /\bvisa\s+sponsorship\s*[:\-]?\s*yes\b/i,
  /\bwe\s+sponsor\b/i,
  /\brelocation\s+and\s+visa\b/i,
  /\bwork\s+permit\s+(?:support|sponsorship|provided)/i,
];

/**
 * Tri-state, deliberately:
 *   true  — the listing says sponsorship is available
 *   false — the listing says it is NOT available
 *   null  — the listing does not mention it
 *
 * The old boolean collapsed "refuses" and "never mentioned" into false, so a
 * filter could not tell them apart. Null lets the caller decide.
 */
function detectVisaSponsorship(plainText) {
  if (!plainText) return null;
  for (const re of VISA_REFUSED) if (re.test(plainText)) return false;
  for (const re of VISA_OFFERED) if (re.test(plainText)) return true;
  // Mentioned but neither clearly offered nor refused — still unknown.
  return null;
}

/**
 * Experience level from the description.
 *
 * Seniority words are checked first, then an explicit years-of-experience range,
 * which is what recovers the large "Software Engineer"-shaped middle that has no
 * seniority word at all: 12,329 of 47,022 active jobs had no level as of
 * 2026-09-24 because only the keyword pass existed and its year pattern was
 * limited to [3-6].
 */
function detectExperienceLevel(plainText) {
  if (!plainText) return null;

  if (/\b(?:director|vp\b|vice\s+president|head\s+of|chief|c[tef]o\b)/i.test(plainText)) return 'EXECUTIVE';
  if (/\b(?:senior|sr\.?|lead|staff|principal|expert)\b/i.test(plainText)) return 'SENIOR';
  if (/\b(?:junior|jr\.?|entry[- ]level|graduate|intern(?:ship)?|trainee|apprentice|working\s+student)\b/i.test(plainText)) return 'ENTRY';
  if (/\bmid[- ]?(?:level|senior)\b|\bintermediate\b/i.test(plainText)) return 'MID';

  // "3+ years", "5-7 years of experience", "minimum 2 years" — take the lower
  // bound, which is what the listing is actually requiring.
  const years = plainText.match(/(\d{1,2})\s*(?:\+|-|–|\s+to\s+)?\s*(?:\d{1,2})?\s*years?(?:['’]|\s+of)?\s+(?:work\s+|relevant\s+|professional\s+)?experience/i);
  if (years) {
    const low = Number(years[1]);
    if (Number.isFinite(low)) {
      if (low <= 2) return 'ENTRY';
      if (low <= 5) return 'MID';
      return 'SENIOR';
    }
  }
