...
const RULES = [
  ...
  { p: /\bproject\s+lead\b|\bdelivery\s+(?:lead|manager|head)\b/i, c: 'Technical Project Manager', w: NEAREST },
  { p: /\bproject\s+(?:excellence|governance|portfolio)\b|\bpmo\b/i, c: 'Technical Project Manager', w: NORMAL },
  ...
  { p: /\bproduct\s+(?:lead|head)\b/i, c: 'Product Manager', w: NORMAL },
  ...
  // Financial-crime roles are compliance work and were classifying to nothing.
  { p: /\bfincrime\b|\bfinancial\s+crime\b|\baml\b|\bkyc\b|\bsanctions\b/i, c: 'Compliance Specialist', w: NORMAL },
  ...
  { p: /\bmarket\s+intelligence\b|\banalyst\s+relations\b|\bcompetitive\s+intelligence\b/i, c: 'Market Research Analyst', w: NORMAL },
  ...
  { p: /\bsap\b|\berp\b|\bservice\s+delivery\s+management\b/i, c: 'IT Consultant', w: DEFAULT },
  ...
  { p: /\bprogramme?\s+(?:lead|director|head)\b/i, c: 'Program Manager', w: NORMAL },
  ...
  // Generic operations fallback: "Head of Business Operations", "KYC
  // Operations Team Lead". Sits at DEFAULT so the specific operations rules
  // above (sales ops, legal ops, ML ops) displace it via the top-k window.
  { p: /\boperations\b|\bops\b/i, c: 'Operations Manager/Director', w: DEFAULT },
