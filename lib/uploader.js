...
async function processScraperResults(jobs) {
  ...
  for (const job of jobs) {
    try {
      ...
      // Atomic upsert: job + category mappings in a single DB transaction
      const { data: result, error: rpcError } = await supabase.rpc('upsert_job_with_categories', {
        p_job: jobRecord,
        p_category_ids: categoryIds,
        p_existing_job_id: existing ? existing.id : null,
      });

      if (rpcError) {
        console.warn(`Failed to ${existing ? 'update' : 'insert'} job "${job.title}": ${rpcError.message}`);
        stats.errors++;
        continue;
      }
