...
function buildJobRecord(job, companyId, sourceId) {
  ...
  return {
    ...
    // `||` collapsed null into false here and silently undid the parser fix —
    // 8,092 rows from run 36024969573 landed with visa_unknown=0 because of it.
    // `??` preserves an explicit false from the parser while keeping null null.
    visa_sponsorship: job.visa_sponsorship ?? parsed.visa_sponsorship ?? null,
    source_id: sourceId,
 ...
 */
/**
 * Postgres rejects an upsert whose payload contains two rows with the same
 * conflict key: "ON CONFLICT DO UPDATE command cannot affect row a second
 * time". One duplicate kills the WHOLE batch — measured 2026-09-23, TokyoDev
 * emitted repeated external_job_ids and lost 132 of 137 jobs inside a run that
 * reported ok. Last occurrence wins, matching what a sequential writer would do.
 */
function dedupeByConflictKey(records) {
  const byKey = new Map();
  for (const r of records) byKey.set(`${r.external_source}\u0000${r.external_job_id}`, r);
  return [...byKey.values()];
}

async function writeJobBatch(rawRecords, stats, label) {
  const records = dedupeByConflictKey(rawRecords);
  if (records.length !== rawRecords.length) {
    console.warn(
      `Batched job ${label}: dropped ${rawRecords.length - records.length} duplicate `
      + 'external_job_id(s) before writing — the source emits repeats',
    );
  }
...
module.exports = { processScraperResults, dedupeByConflictKey };
