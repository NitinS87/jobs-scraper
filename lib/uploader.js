...
async function fetchCompanyDetails(ids) {
  ...
  for (const slice of chunk(ids, LOOKUP_CHUNK_SIZE)) {
    const { data, error } = await supabase
      .from('companies')
...
async function prefetchExisting(jobs, hasContentHash) {
  ...
  for (const slice of chunk(jobIds, LOOKUP_CHUNK_SIZE)) {
    const { data, error } = await supabase
...
async function resolveInsertedIds(records) {
  ...
  for (const slice of chunk(ids, LOOKUP_CHUNK_SIZE)) {
    ...
    if (error) {
      console.warn(`Failed to resolve inserted job ids: ${error.message}`);
      return out;
    }
    for (const row of data || []) {
